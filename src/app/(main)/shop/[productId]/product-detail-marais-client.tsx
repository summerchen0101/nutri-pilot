...
export function ProductDetailMaraisClient({
  ...
  return (
    <>
        ...
        <div className="space-y-3 p-4">
          ...
          <div className="flex items-center gap-3 rounded-xl bg-secondary/80 px-3 py-2.5">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-body font-medium text-foreground">{brand.name}</p>
      ...
      <div
        className="sticky z-40 -mx-4 mb-3 border-b-hairline border-border/70 bg-background/90 px-4 py-2 backdrop-blur-md top-[calc(env(safe-area-inset-top)+3.25rem)]"
      >
        <div className="flex gap-6" role="tablist" aria-label="商品資訊分頁">
          {TAB_KEYS.map((key) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'relative pb-2 text-body text-muted-foreground transition-colors',
              ...
              >
                ...
                {active ?
                  <span
                    className="absolute inset-x-0 bottom-0 h-0.5 rounded-full bg-[var(--steel-accent)]"
                    aria-hidden
                  />
                : null}
              </button>
      ...
      <div className="space-y-4 pb-28">
              ...
              <ul className="mt-3 space-y-2">
                {fitReasons.map((r, i) => (
                  ...
                  >
                    ...
                    <span
                      className={cnReason(r.type, 'min-w-0 flex-1')}
                    >
                      {r.text}
                    </span>
                  </li>
                ...
                <div className="hide-scrollbar mt-3 flex gap-3 overflow-x-auto pb-1 [-webkit-overflow-scrolling:touch]">
                  {sameBrand.map((sp) => {
                    const variantsPrices = sp.variants as
                      ...
                      | null;
                    const minP =
                      variantsPrices?.length ?
                        Math.min(...variantsPrices.map((v) => Number(v.price)))
                      : 0;
                    return (
                      <Link
                        key={sp.id}
        ...
        {detailTab === 'delivery' ?
          <section className="rounded-xl bg-card p-4 space-y-3">
            ...
            <ul className="space-y-2 text-body text-muted-foreground">
              <li>
                運費：NT${' '}
                {formatShopGroupedInteger(vendor.shippingFee)}
                （結帳時依購物車合併計算）
              </li>
              <li>
                {vendor.freeShippingThreshold != null ?
                  <>
                    滿 NT${' '}
                    {formatShopGroupedInteger(vendor.freeShippingThreshold)} 免運
                    （依單一廠商門檻）
                  </>
                : (
                  '免運門檻請見結帳頁說明'
        ...
        {detailTab === 'payment' ?
          <section className="rounded-xl bg-card p-4 space-y-2">
            <p className="text-body leading-relaxed text-foreground">
              結帳時支援藍新金流 MPG 信用卡一次付清；實際可用付款方式依結帳頁為準。
            </p>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-0 z-[42] px-4 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2">
        <div className="pointer-events-auto mx-auto flex max-w-sm items-end gap-3 border-t-hairline border-border bg-[var(--color-background-primary)]/95 px-3 py-2 backdrop-blur-md">
