...
export default async function SettingsOrdersPage() {
  ...
  const { data: rows, error } = await supabase
    .from('orders')
    .select('id, public_order_no, status, total, created_at')
    .eq('user_id', user.id)
          ...
          {list.map((row) => (
            <li key={row.id}>
              <Link
                ...
                className={cn(
                  'block rounded-xl border-hairline border-border bg-card px-3 py-3',
                  'transition-colors hover:border-[#4C956C]/40',
                )}
              >
