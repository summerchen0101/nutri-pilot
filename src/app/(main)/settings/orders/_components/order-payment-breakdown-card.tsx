...
export interface OrderPaymentBreakdownCardProps {
...
export function OrderPaymentBreakdownCard({
  ...
  return (
    <section className="rounded-xl border-hairline border-border bg-card p-4">
      ...
      >
        {vendors.map((vendor) => (
          <li key={vendor.vendorId} className="space-y-2">
            <p className="text-caption font-medium text-foreground">
              {vendor.vendorName}
            </p>
            {vendor.selectedShippingMethodLabel ?
              <p className="mt-1 text-caption text-muted-foreground">
                運送：{vendor.selectedShippingMethodLabel}
              </p>
            : null}
            ...
            <div className="mt-2 flex flex-wrap justify-between gap-2 text-body text-muted-foreground">
              <span>小計</span>
              <span className="tabular-nums text-foreground">
                NT$ {formatShopGroupedInteger(vendor.itemsSubtotal)}
              </span>
            </div>
            <div className="mt-1 flex flex-wrap justify-between gap-2 text-body">
              <span className="text-muted-foreground">運費</span>
              <span className="tabular-nums text-foreground">
                NT$ {formatShopGroupedInteger(vendor.effectiveShipping)}
                {vendor.gapToFreeShipping != null && vendor.effectiveShipping > 0 ?
                  `（差 NT$${formatShopGroupedInteger(vendor.gapToFreeShipping)} 享免運）`
                : vendor.effectiveShipping === 0 ?
                  '（已達免運）'
                : null}
              </span>
            </div>
