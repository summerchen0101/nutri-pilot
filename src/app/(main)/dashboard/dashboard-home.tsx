...
function WeeklyTrendCard({
  ...
  const weightPoints = weeklyWeight
    .map((row, idx) => {
      ...
      const x = weeklyWeight.length <= 1 ? 50 : (idx / (weeklyWeight.length - 1)) * 100;
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        <div className="rounded-[10px] border-[0.5px] border-border bg-secondary/40 p-3">
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">
            {validWeights.length > 0 ? `${validWeights[validWeights.length - 1]} kg` : '—'}
          </p>
        </div>
        <div className="rounded-[10px] border-[0.5px] border-border bg-secondary/40 p-3">
              ...
              <div key={row.label} className="flex min-w-0 flex-1 flex-col items-center gap-1">
                <div className="h-10 w-full overflow-hidden rounded-[4px] bg-muted">
                  <div
                    ...
                    style={{ height: `${Math.max(8, (row.kcal / maxKcal) * 100)}%` }}
                  />
                </div>
              </div>
            ))}
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">
            平均 {Math.round(weeklyKcal.reduce((sum, row) => sum + row.kcal, 0) / Math.max(1, weeklyKcal.length))} kcal
          </p>
        </div>
      </div>
    </SectionCard>
      ...
      <ul className="mt-3 space-y-2">
        {bullets.map((text, idx) => (
          <li key={`${idx}-${text.slice(0, 8)}`} className="flex gap-2">
            ...
            <span className="text-[13px] leading-relaxed text-foreground">{text}</span>
          </li>
        ))}
      </ul>
    </SectionCard>
      ...
      <div className="mt-3 flex gap-2.5 overflow-x-auto pb-1 [-webkit-overflow-scrolling:touch]">
        {products.map((product) => (
          <Link
            ...
            className="min-w-[146px] shrink-0 overflow-hidden rounded-xl border-[0.5px] border-border bg-card"
          >
            <div className="relative aspect-square bg-muted">
              {product.imageUrl ? (
                <Image src={product.imageUrl} alt="" fill sizes="150px" className="object-cover" unoptimized />
              ) : null}
            </div>
            <div className="p-2.5">
              ...
              <p className="mt-1 line-clamp-1 text-[11px] text-muted-foreground">{product.reason}</p>
...
export function DashboardHome({
  ...
  const displayMeals = mealSlots.map((slot) => {
    ...
    return {
      ...
      recordHref: existing?.recordHref ?? `/log?meal_type=${encodeURIComponent(slot.key)}`,
    };
  });
  ...
  return (
    <div className="space-y-3">
      <section className="flex items-center justify-between gap-2">
        <span className="inline-flex shrink-0 rounded-full bg-primary-light px-2.5 py-0.5 text-[11px] font-medium text-primary-foreground">
          連續 {streakDays} 天
        </span>
            ...
            goal != null ? (
              <>
                ...
                {calorieRemainKcal != null ? (
                  <>
                    {' · 尚可攝取約 '}
                    <span className="text-primary">{calorieRemainKcal} kcal</span>
                  </>
                ) : null}
              </>
            ) : (
