...
/** 與 prompt／lookups 對齊；用於從 risk_items 補 alert_keywords */
...
/**
 * 就地補強 AI 報告：高鈉警示、添加物 alert 補詞、補 high tier 時微調安全分數。
 */
...
/** 前端顯示用：依 tier 嚴重度排序 risk_items（不 mutate 原陣列）。 */
export function sortRiskItemsByTier<T extends { tier: string }>(items: T[]): T[] {
