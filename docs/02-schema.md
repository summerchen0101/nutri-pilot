...
- **`user_profiles.tracks_glycemic_concern`**：BOOLEAN，預設 FALSE；標示守衛流程仍可查該欄以調整高糖提示強度。**應用層已不再提供設定頁開關**；新使用者不經 UI 異動時維持預設。（若需相關留意，請以口述健康脈絡寫入 `personal_context_facets`。）
...
- **`label_guard_saved_reports`**（migration `018_label_guard_saved_reports.sql`）：食品安全守衛結果可存為「個人紀錄」，欄位包含 `name`、`report_json`、`job_id`；RLS 僅允許本人存取，DB trigger 限制每位使用者最多 5 筆。
