...
### P1-1 Supabase 專案建立
- [ ] 建立 Supabase 專案
- [ ] 執行 `02-schema.md` 的所有 SQL（依序執行）
- [ ] 啟用所有表的 RLS 並設定基本政策
- [ ] 設定 Supabase Auth（Email + Magic Link）
- [ ] 執行 `supabase gen types typescript` 產生型別

### P1-2 Next.js 基礎架構
- [ ] Next.js 14 App Router + TypeScript + Tailwind CSS
- [ ] 設定 `lib/supabase/server.ts` 和 `lib/supabase/client.ts`
- [ ] 設定 middleware（保護 `/(main)` 路由）
...
export function calcBMR(gender: string, birthDate: Date, heightCm: number, weightKg: number): number
// Mifflin-St Jeor 公式
...
export function calcDailyCalTarget(tdee: number, goalType: string, weeklyRateKg: number): number
// lose_weight：tdee - (weeklyRateKg × 7700 / 7)
// gain_muscle：tdee + (weeklyRateKg × 7700 / 7)
// maintain：tdee

export function calcTargetDate(currentWeight: number, targetWeight: number, weeklyRate: number): Date
// 相差公斤 / weeklyRate × 7 天

export function calcRecommendScore(product: Product, profile: UserProfile, goal: UserGoal): number
// 見 05-shop.md 的完整邏輯
```
